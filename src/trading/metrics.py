...
from .exit_strategies import (
    calculate_dynamic_exits,
    confidence_based_position_sizing,
    consensus_exit_rules,
...
def calculate_enhanced_meta_trading_metrics_with_exits(
    ...
    if not use_exit_rules:
        ...
        for i in range(len(y_true_np)):
            ...
            if position_ids and use_exit_rules:
                ...
                if config.exit_rules.position_management.enable_consensus_exits and i >= smooth_window:
                ...
                if trailing_stop:
                    current_profits = [
                        (current_price - pos["entry_price"]) * pos["direction"] * pos["size"]
                        - cost_per_mwh * pos["size"]
                ...
                for j, pos_id in enumerate(position_ids):
                    if exit_mask[j] or pos_id in trailing_exits:
                        ...
                        exit_pnl = (current_price - pos["entry_price"]) * pos["direction"] * pos[
                            "size"
                        ] - cost_per_mwh * pos["size"]
    ...
    if use_exit_rules:
        ...
        avg_position_size = (
            np.mean(position_sizes[consensus_trade_mask == 1]) if np.sum(consensus_trade_mask) > 0 else 0
        )
